...
<section class="snipp-hq-less">
    ...
    <table class="table-responsive table table-striped table-sm" style="background: #e6e6e6">
        ...
        {
            $keyword = $_POST['search_link'];
            require_once ( SNIPP_BASE . 'db/dbh.php');
            $sql = "SELECT * FROM tsw_snippets WHERE `anchor` LIKE :keyword";
            $stmt = $dbh->prepare($sql);
            $stmt->bindValue(':keyword', '%' . $keyword . '%');
            $result = $stmt->execute();
        ...
        ?>
        <tr><td colspan="3"><a href="#body" class="btn btn-sm" title="back to top">back to top</a></td></tr>
    </tbody>
    </table>
</section>
</article>
